package com.metropolitana.multipagos.forms.gestion_banpro;

import com.metropolitana.multipagos.DetalleGBanpro;
...
import com.metropolitana.multipagos.GestionBanpro;
import com.metropolitana.multipagos.SimboloBanpro;
...
import com.metropolitana.multipagos.forms.simbolo.SimboloBanproHandler;
import org.apache.ojb.broker.query.ReportQueryByCriteria;

public class GestionBanproHandler {

	/**
	 * <code>VISITA_ID</code> Identificador del Control de GestionBanpro
	 */
	 ...
	 * @throws Exception
	 */
	public void insert(final GestionBanpro bean, Integer usrId) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void update(final GestionBanpro bean, Integer usrId)
			throws Exception {
		PersistenceBroker broker = null;
	...
	public void remove(Integer gestionId, Integer usrId) throws Exception {
        ...
        try {
            ...
            broker.beginTransaction();
            //Date fecha = Calendar.getInstance().getTime();
            GestionBanpro bean = retrieve(gestionId, broker);
            broker.delete(bean);
	 ...
	 */
	public static GestionBanpro retrieve(final Integer gestionId,
			final PersistenceBroker broker) {
		GestionBanpro criterio = new GestionBanpro();
		criterio.setGestionId(gestionId);
		Query query = new QueryByIdentity(criterio);
		return (GestionBanpro) broker.getObjectByQuery(query);
	}
	 ...
	 * @param broker
	 */
	private void actualizarReferencias(GestionBanpro gestion, PersistenceBroker broker) {
		gestion.setUsrIdRef(Auth_userHandler.retrieve(gestion.getUsrId(), broker));
		linkChilds(gestion, broker);
	}

	private void linkChilds(final GestionBanpro bean, final PersistenceBroker broker) {
		Iterator iterDetalle = bean.getDetalleGBanproList().iterator();
		while (iterDetalle.hasNext()) {
			DetalleGBanpro det = (DetalleGBanpro) iterDetalle.next();
                        det.setCiudadCIdRef(CiudadHandler.retrieve(det.getCiudadCId(), broker));
                        det.setDepartamentoIdRef(BDepartamentosHandler.retrieve(det.getDepartamentoId(), broker));
			det.setTmpIdRef(CarteraBanproHandler.retrieve(det.getTmpId(), broker));
                        ...
                        det.setSimboloIdRef(SimboloBanproHandler.retrieve(det.getSimboloId(), broker));
	...
	/**
	 * Genera listado de GestionBanpro.
	 * 
	 * @param criterio
	 *            Criterio de filtrado de la lista
	 * @return Collection o null listado de GestionBanpro
	 * @throws Exception
	 */
	private Collection getList(final Criteria criterio) throws Exception {
		...
		try {
			broker = PersistenceBrokerFactory.defaultPersistenceBroker();
			QueryByCriteria query = new QueryByCriteria(GestionBanpro.class,
					criterio);
	...
	public final Collection<GestionBanpro> getResultadosXPagina(
	...
	/**
	 * Obtiene la cantidad de GestionBanpro que coinciden con el filtro de
	 * búsqueda.
	 * 
	 * @param visitaNombre
	 *            Filtro de búsqueda.
	...
	/**
	 * Genera el listado de todos los GestionBanpro.
	 * 
	 * @return Collection o null Listao de GestionBanpro
	 * @throws Exception
	 */
	...
	private static List getNombreList(Criteria criterio) throws Exception {
        ...
        try {
            broker = PersistenceBrokerFactory.defaultPersistenceBroker();
            QueryByCriteria query = new QueryByCriteria(DetalleGBanpro.class, criterio);
	...
	public Date getUltimaVisita(String codigo) throws Exception {
        ...
        try {
            ...
            ReportQueryByCriteria query = new ReportQueryByCriteria(DetalleGBanpro.class, criterio);
	...
	public static boolean getTarjetaXFecha(final Boolean gestionLlamada,
			final String tarjeta, final Date fechaGestion)
			throws Exception {
		PersistenceBroker broker = null;
		try {
			broker = PersistenceBrokerFactory.defaultPersistenceBroker();
			...
			if (tarjeta != null && fechaGestion != null) {
				criterio.addEqualTo("tarjeta", tarjeta);
				criterio.addEqualTo("fechaGestion", fechaGestion);
				
			}
			List lst = getTarjetaXFechaList(criterio);
			if (lst.isEmpty()) {
				return Boolean.FALSE.booleanValue();
			} else {
				return Boolean.TRUE.booleanValue();
	...
	private static List getTarjetaXFechaList(Criteria criterio) throws Exception {
        PersistenceBroker broker = null;
