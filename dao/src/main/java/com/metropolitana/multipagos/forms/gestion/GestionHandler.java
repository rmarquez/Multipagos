package com.metropolitana.multipagos.forms.gestion;
...
public class GestionHandler {
	...
	/**
	 * <code>VISITA_ID</code> Identificador del Control de GestionAvon
	 */
	 ...
	 * @throws Exception
	 */
	public void insert(final GestionAvon bean, Integer usrId) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void update(final GestionAvon bean, Integer usrId)
			throws Exception {
		PersistenceBroker broker = null;
	...
	public void remove(Integer gestionId, Integer usrId) throws Exception {
        ...
        try {
            ...
            broker.beginTransaction();
            //Date fecha = Calendar.getInstance().getTime();
            GestionAvon bean = retrieve(gestionId, broker);
            broker.delete(bean);
	 ...
	 */
	public static GestionAvon retrieve(final Integer gestionId,
			final PersistenceBroker broker) {
		GestionAvon criterio = new GestionAvon();
		criterio.setGestionId(gestionId);
		Query query = new QueryByIdentity(criterio);
		return (GestionAvon) broker.getObjectByQuery(query);
	}
	 ...
	 * @param broker
	 */
	private void actualizarReferencias(GestionAvon gestion, PersistenceBroker broker) {
		gestion.setUsrIdRef(Auth_userHandler.retrieve(gestion.getUsrId(), broker));
		linkChilds(gestion, broker);
	}

	private void linkChilds(final GestionAvon bean, final PersistenceBroker broker) {
		Iterator iterDetalle = bean.getDetalleGestionList().iterator();
		while (iterDetalle.hasNext()) {
			DetalleGestion det = (DetalleGestion) iterDetalle.next();
			
			det.setCavonIdRef(CarteraAvonHandler.retrieve(
					det.getCavonId(), broker));
			det.setSimboloIdRef(SimboloAvonHandler.retrieve(
	...
	/**
	 * Genera listado de GestionAvon.
	 * 
	 * @param criterio
	 *            Criterio de filtrado de la lista
	 * @return Collection o null listado de GestionAvon
	 * @throws Exception
	 */
	private Collection getList(final Criteria criterio) throws Exception {
		...
		try {
			broker = PersistenceBrokerFactory.defaultPersistenceBroker();
			QueryByCriteria query = new QueryByCriteria(GestionAvon.class,
					criterio);
	...
	public final Collection<GestionAvon> getResultadosXPagina(
	...
	/**
	 * Obtiene la cantidad de GestionAvon que coinciden con el filtro de
	 * búsqueda.
	 * 
	 * @param visitaNombre
	 *            Filtro de búsqueda.
	...
	/**
	 * Genera el listado de todos los GestionAvon.
	 * 
	 * @return Collection o null Listao de GestionAvon
	 * @throws Exception
	 */
	...
	public Date getUltimaVisita(String codigo) throws Exception {
        ...
        try {
            ...
            ReportQueryByCriteria query = new ReportQueryByCriteria(DetalleGestion.class, criterio);
