<?xml version="1.0"?>
<!-- Hoja para la generación de ticket en formato PDF -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
        ...
        <fo:page-sequence force-page-count="no-force" master-reference="pages">
            ...
            <xsl:if test="@page-number-top">
                <fo:static-content flow-name="xsl-region-before">
                    <fo:block font-size="10pt" text-align="right" line-height="14pt"><fo:page-number/> de <fo:page-number-citation ref-id="last-page"/></fo:block>
                </fo:static-content>
            </xsl:if>
             Indicamos que el número de página lo queremos en el final de la página
            <xsl:if test="@page-number-bottom">
                <fo:static-content flow-name="xsl-region-after">
                    <fo:block font-size="10pt" text-align="center" line-height="14pt"><fo:page-number/> de <fo:page-number-citation ref-id="last-page"/></fo:block>
                </fo:static-content>
            </xsl:if>
    ...
    <xsl:template match="encabezado">
        <fo:block font-size="10pt" font-weight="bold" space-before.optimum="14pt" text-align="center"
    ...
    <xsl:template match="texto">
        <fo:block space-before.precedence="10" space-after.precedence="10" text-align="justify" line-height="14pt">
            <xsl:choose>
    ...
    <xsl:template match="titulo|espacio">
        <fo:block font-size="10pt" font-weight="bold" font-family="serif" space-before.optimum="6pt" line-height="14pt">
    ...
    <xsl:template match="row">
        <fo:block font-size="10pt" space-before.optimum="8pt" text-align="left" >
            <xsl:apply-templates/>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template match="b">
        <fo:inline font-weight="bold" font-family="serif">
            <xsl:apply-templates/>
        </fo:inline>
    </xsl:template>

    <xsl:template match="line">
        <fo:block font-size="10pt" space-before.optimum="10" text-align="left">
            <xsl:apply-templates/>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template match="th">
        <fo:table-cell font-family="serif">
    ...
    <xsl:template match="td">
        <fo:table-cell border-collapse="collapse" font-family="serif">
            <xsl:if test="ancestor::table[1]/@border > 0 or @border > 0">
                <xsl:attribute name="border-style">solid</xsl:attribute>
                <xsl:attribute name="border-width">0.1pt</xsl:attribute>
            </xsl:if>
            ...
            <!-- La propiedad hyphenate="true" hace que el contenido de la columna, cuando es demasiado
                 largo para su ancho, salte a la siguiente línea.
            -->
            <fo:block font-size="10pt" hyphenate="true" language="es">
            <!--    configuramos alineación para que sea igual al de la etiqueta <td>-->
                <xsl:choose>
                    <xsl:when test="@align='left'">
                        <xsl:attribute name="text-align">start</xsl:attribute>
